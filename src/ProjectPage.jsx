    ...
    <div style={{ background: BG, minHeight: "100vh", fontFamily: "Inter,sans-serif" }}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .img-thumb { cursor: pointer; opacity: 0.55; transition: opacity 0.2s, transform 0.2s; border: 2px solid transparent; }
        .img-thumb.active { opacity: 1; border-color: ${ACCENT}; }
      ...
      <div style={{
        ...
        <button onClick={() => navigate("/")} style={{
          ...
          color: "rgba(255,255,255,0.6)", borderRadius: 8, padding: "8px 18px",
      ...
      {/* Hero */}
      <div style={{
        height: "60vh", minHeight: 400, marginTop: 64, overflow: "hidden", position: "relative",
        ...
        <div style={{
          position: "absolute", bottom: 48, left: "5%", right: "5%",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 5% 120px" }}>
        ...
        {p.images.length > 0 && (
          <div className="pg-fade in" style={{ marginBottom: 80 }}>
            ...
            <div style={{ width: "100%", aspectRatio: "16/9", borderRadius: 16, overflow: "hidden", background: CARD_BG, border: `1px solid ${BORDER}`, marginBottom: 16, position: "relative" }}>
              <img src={p.images[imgIdx].src} alt={p.images[imgIdx].caption}
                style={{ width: "100%", height: "100%", display: "block", transition: "opacity 0.3s", ...p.images[imgIdx].style }} />
              {p.images[imgIdx].caption && (
                <div style={{ position: "absolute", bottom: 16, left: 20, fontFamily: "Inter,sans-serif", fontSize: 13, color: "rgba(255,255,255,0.5)" }}>
                  {p.images[imgIdx].caption}
                </div>
              )}
            </div>
            ...
            {p.images.length > 1 && (
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {p.images.map((img, i) => (
                  <div key={i} className={`img-thumb${i === imgIdx ? " active" : ""}`}
                    onClick={() => setImgIdx(i)}
        ...
        {/* Challenge & Approach */}
        <div className="pg-fade in" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginBottom: 64 }} className="detail-grid">
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {p.tools.map(t => (
              <span key={t} style={{
                ...
                border: `1px solid rgba(203,172,249,0.2)`, borderRadius: 8,
                padding: "8px 18px", background: "rgba(203,172,249,0.06)"
              }}>{t}</span>
            ))}
          </div>
        </div>
